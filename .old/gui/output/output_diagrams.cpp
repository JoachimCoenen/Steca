...
void DiagramsFrame::writeCurrentDiagramOutputFile(rcstr filePath, rcstr separator) {
  ...
  EXPECT_(xs_.count() == ys_.count())
  EXPECT_(ysErrorLo_.isEmpty() || ysErrorLo_.count() == ys_.count())
  EXPECT_(ysErrorLo_.count() == ysErrorUp_.count())
